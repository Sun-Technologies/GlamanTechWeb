<?php $CURRENT_PAGE= "employees"; ?>
<?php include'header.php'; ?>
  <div id="primary" class="container   clearfix animated bounceIn" style="padding-bottom: 25%;">
   <h1 id="side-nav" class="page-title navbar-example">My HR</h1>
       <hr/>
      <div class="wp-area">
      <ul class="my-hr-box">
        <li><h3 style="text-align: center;">Form W4</h3> <a href="http://www.irs.gov/pub/irs-pdf/fw4.pdf" target="_blank"><br>http://www.irs.gov/pub/irs-pdf/fw4.pdf </a></li>
        <li><h3 style="text-align: center;">Form I9</h3> <a href="http://www.uscis.gov/sites/default/files/files/form/i-9.pdf" target="_blank"><br>http://www.uscis.gov/sites
          /default/files/files/form/i-9.pdf</a></li>
        <li><h3 style="text-align: center;">ADP Payroll Link</h3> <a href="http://myaccess.adp.com/" target="_blank"><br>http://myaccess.adp.com/</a></li>
      </ul>
      </div>
  </div>
<?php include'footer1.php'; ?>